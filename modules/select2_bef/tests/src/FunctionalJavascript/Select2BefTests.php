...
class Select2BefTests extends Select2BoxesTestsBase {
  ...
  public function testBetterExposedFormPluginOverride() {
    ...
    $this->assertTrue($plugin_manager->hasDefinition('select2_bef'));

    $class_name = 'Drupal\select2_bef\Plugin\views\exposed_form\Select2BEF';
    $select2_bef_definition = $plugin_manager->getDefinition('select2_bef');

    $this->assertEquals($class_name, $select2_bef_definition['class']);
  }
  ...
  public function testEntityReferenceFieldFilters() {
    ...
    $dropdown = $this->minkSession
      ->getPage()
      ->find('xpath', '//select[@name="exposed_form_options[bef][filter][field_tags_target_id][configuration][plugin_id]"]');
    ...
    $this->minkSession
      ->getPage()
      ->selectFieldOption(
        'exposed_form_options[bef][filter][field_tags_target_id][configuration][plugin_id]',
    ...
    $this->minkSession
      ->getPage()
      ->checkField('exposed_form_options[bef][filter][field_tags_target_id][configuration][advanced][reference_bundles][tags]');
